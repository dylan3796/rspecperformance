...
> **Current state: holding page.** The public site is deliberately down to a single "Work in progress" line (`app/page.tsx`) — no logo, no chrome. The full homepage is parked, intact, in `components/home/HomePage.tsx`, and `<Nav />` / `<Footer />` are commented out of `app/layout.tsx`. `/admin` and `/portal` still work. To put the site back: re-export the parked page from `app/page.tsx` and restore the two layout components. Don't rebuild the homepage from scratch — it already exists.
